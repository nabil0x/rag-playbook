...
- Every pipeline, lab, and notebook embeds with a **local model**:
  ...
  `embeddings/bge.py`) or E5 (`embeddings/e5.py`).
...
- If LangChain does not support an operation, build a custom class
  implementing the same contract and put it in `tools/` (shared, reusable —
  not notebook-embedded). See `tools/README.md` for the inventory.
- Reuse the repo's component library before writing anything:
  `loaders/`, `splitters/`, `embeddings/`, `vectordb/`, `retrieval/`,
  `prompts/`, `llms/`, `evaluation/`.

## 4. Data

- Fresh corpora live in `Data/corpus/`; provenance is
  `Data/.corpus-manifest.txt` (`url|sha256|bytes|relpath` per line). Never
  re-download what the manifest verifies; use `scripts/fetch_fresh_corpus.py`
  for new/forced fetches (`--mind` gates the research-licensed MIND set).
- SD samples: `Data/.samples-manifest.txt` + `scripts/fetch_sd_samples.py`.
- Corpus map: rag-mini-wikipedia (core), beir-fiqa/nfcorpus (qrels for
  ...
  `scripts/fetch_gutenberg.py`, manifest-verified like the rest).
...
- Generation metrics use gold answers: `rag-mini-wikipedia/test.parquet`,
  `hotpotqa` dev set, `scifact` claims — via `evaluation/`.
...
- P20–P36 module stubs, Topics cards, `Data/corpus/`, and
  `scripts/fetch_fresh_corpus.py` are currently **uncommitted**. Nothing is
